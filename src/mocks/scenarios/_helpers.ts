...
import { computeReplenishmentRequests } from "../engine"
import { isoOffset } from "../clock"

export function scaledCapacityAllocations(
  factor: number
): CapacityAllocation[] {
  ...
  for (const store of selectedStores) {
    ...
    for (const sku of selectedSkus) {
      ...
      if (storeIsDue && roll < 0.18) {
        daysOfCover =
          Math.round((rng() * (sku.reorder_threshold_days - 1) + 0.5) * 10) / 10
      } else {
        daysOfCover =
          Math.round((sku.reorder_threshold_days + 2 + rng() * 10) * 10) / 10
      }
      const onHand = Math.max(
        0,
        Math.round(daysOfCover * sku.default_burn_per_day)
      )
      rows.push({
        store_id: store.id,
        sku_id: sku.id,
...
export function buildScenario(opts: ScenarioBuildOptions): BuiltScenario {
  const inferred = buildInferredStock(
    stores,
    skus,
    opts.draftStoreCount,
    opts.seed
  )
  const engine = computeReplenishmentRequests({
    stores,
    skus,
